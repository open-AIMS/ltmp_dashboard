...
#+END_SRC
* CSV files
** view first six rows in neat columns (scrollable left and right)
head fish.csv | colun -s "," -t -o "|" | less -S
** view first six rows in neat columns excluding first column (scrollable left and right)
head fish.csv | colun -s "," -t -o "|" H 1 | less -S
* Cronjob
** format
      ┌───────────── minute (0 - 59)
      │ ┌───────────── hour (0 - 23)
...
*
* Batch
sudo -u shiny Rscript ~/dev/R/batch.R --purpose=fit --method=photo-transect --scale=reef --domain='Fairfax Island' --log=../../data/dashboard.log >> ~/crontab.log 2>&1
- purpose=sql
#+BEGIN_SRC bash
...
* Create apptainer
 – Installing apptainer
   follow the instructions https://apptainer.org/docs/admin/1.0/installation.html#install-from-source
 ...
 – Fix permission issues with inla.mkl.run etc
   – create sandbox
   ...
   – enter sandbox with writable permissions
     ...
     chmod +x /usr/local/lib/R/site-library/INLA/bin/linux/64bit/inla.mkl
     exit
   – rebuild image
     apptainer build ltmp_dashboard1.sif ltmp_dashboard_sandbox.sif
   – scp to HPC
     scp ltmp_dashboard1.sif hpc:~/Projects/LTMP_dashboard
   – clone the ltmp_dashboard repo
     git clone git@github.com:open-AIMS/ltmp_dashboard.git
     git checkout dev
   – running apptainer / singularity on the hpc (from ~/Projects/LTMP_dashboard)
   singularity exec -B ltmp_dashboard:/home/Project -B data:/data -B /etc/localtime:/etc/localtime -B /etc/timezone:/etc/timezone --pwd /home/Project/R ltmp_dashboard1.sif Rscript 00_main.R --path=/data/raw/reef_data.zip --method=fish --domain=Dip Reef --scale=reef --status=true --refresh_data=false
